    ...
    Cartman.prototype = {
        ...
        add : function(item) {
            ...
            if(item.color === undefined) {
                throw new Error("Item does not have color");
            }
            ...
            var existingRecord = this.get(item.color,item.name);
            var existingCount;
        ...
        getItemColor : function(item) {
            return item[this.fields.color];
        },
        getItemCount : function(item) {
            return item[this.fields.count];
        },
        getItemName : function(item) {
            return item[this.fields.name];
        },
        /**
         ...
         */
        get : function(color,name) {
            var i;
            for(i=0; i<this.contents.length; i++) {
                var item = this.contents[i];
                var itemColor = this.getItemColor(item);
                var itemName = this.getItemName(item);
                if(itemColor == color && itemName == name) {
                    return item;
                }
            }
            return null;
